...
export default function EditMedicineScreen() {
  ...
  const loadMedicine = async () => {
    try {
      const medicine = await medicinesApi.get(id!);
      ...
      setStartDate(new Date(medicine.start_date));
      if (medicine.end_date) {
        setEndDate(new Date(medicine.end_date));
        setIsOngoing(false);
      } else {
        setIsOngoing(true);
      }
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      const updatedMedicine = await medicinesApi.update(id!, {
        name: name.trim(),
      ...
      await scheduleMedicineNotifications(updatedMedicine);

      Alert.alert('Success', 'Medicine updated with reminders', [
