...
import { therapyApi, TherapySession } from '../services/api';
...
export default function EditSessionScreen() {
  ...
  const loadSession = async () => {
    try {
      const session = await therapyApi.getSession(id!);
      setTherapyType(session.therapy_type);
      ...
      if (!DURATION_PRESETS.includes(session.duration_minutes)) {
        setCustomDuration(session.duration_minutes.toString());
      }
      setNotes(session.notes || '');
      setFeelingRating(session.feeling_rating);
      setFeelingNotes(session.feeling_notes || '');
    } catch (error: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await therapyApi.updateSession(id!, {
  ...
  const handleDelete = () => {
    const typeConfig = THERAPY_TYPES.find((t) => t.id === therapyType);
    Alert.alert(
      'Delete Session',
      `Are you sure you want to delete this ${typeConfig?.label} therapy session? This action cannot be undone.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setIsLoading(true);
            try {
              await therapyApi.deleteSession(id!);
              router.back();
            } catch (error: any) {
